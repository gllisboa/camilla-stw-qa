...
export class CommentRepo implements ICommentRepo {
  ...
  async getNumberOfCommentsOnDate(date: Date | string): Promise<number> {
    const result = await this.models.sequelize.query(
      `SELECT COUNT(*) AS NUM_COMMENTS FROM comment WHERE created_at like "%${date}%";`
    );
    const count = result[0][0]["NUM_COMMENTS"];
    return count;
  }

  async getNumberOfCommentsMembersOnDate(date: Date | string): Promise<number> {
    const result = await this.models.sequelize.query(
      `SELECT count(*) AS NUM_COMMENTS_MEMBERS FROM comment INNER JOIN member ON comment.member_id = member.member_id WHERE comment.created_at like "%${date}%" group by comment.member_id;`
    );
    if (result[0] === null || undefined) return 1;
